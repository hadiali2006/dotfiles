...
----@class lsp.Client.filter
----@field id? number
----@field bufnr? number
----@field name? string
----@field method? string
----@field filter? fun(client: lsp.Client): boolean
...
function M.get_clients(opts)
    ...
    if vim.lsp.get_clients then
        ...
        if opts and opts.method then
            ret = vim.tbl_filter(function(client) ---@param client lsp.Client
...
function M.formatter(opts)
    ...
    local ret = {
        ...
        primary = true,
        priority = 1,
        format = function(buf) M.format(lazy_utils.merge({}, filter, { bufnr = buf })) end,
        sources = function(buf)
            local clients = M.get_clients(lazy_utils.merge({}, filter, { bufnr = buf }))
            local ret = vim.tbl_filter(function(client) ---@param client lsp.Client
                return client.supports_method("textDocument/formatting")
                    or client.supports_method("textDocument/rangeFormatting")
            end, clients)
            return vim.tbl_map(
                function(client) return client.name end, ---@param client lsp.Client
                ret
            )
        end,
    }
...
function M.format(opts)
    opts = vim.tbl_deep_extend(
        ...
        {},
        opts or {},
        general_utils.get_plugin_opts("nvim-lspconfig").format or {},
        general_utils.get_plugin_opts("conform.nvim").format or {}
    )
...
function M.words.at(words)
    for idx, word in ipairs(words or M.words.get()) do
        if word.current then return word, idx end
    end
end

...
local isFloating = function(client)
    return (client.floatinig or
...
client.connect_signal("request::default_keybindings", function()
    awful.keyboard.append_client_keybindings({
        ...
        -- Resize windows
        awful.key({ modkey, mod.ctrl }, "Up", function(c)
            if isFloating(c) then
                c:relative_move(10, 0, 0, -10)
            else
                awful.client.incwfact(0.025)
            end
        end, { description = "Floating Resize Vertical -", group = "client" }),
        awful.key({ modkey, mod.ctrl }, "Down", function(c)
            if  isFloating(c) then
        ...
        awful.key({ modkey, mod.ctrl }, "Left", function(c)
            if isFloating(c) then
        ...
        awful.key({ modkey, mod.ctrl }, "Right", function(c)
            if isFloating(c) then
                c:relative_move(0, 0, 10, 0)
            else
                awful.tag.incmwfact(0.025)
